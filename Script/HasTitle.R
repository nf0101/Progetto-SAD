...
# Raggruppa per `label` (0 = phishing, 1 = legittimo) e `HasTitle`, quindi calcola i totali
title_summary <- combined_dataset %>%
  group_by(label, HasTitle) %>%
...
title_summary <- title_summary %>%
  mutate(
    ...
    title_Status = ifelse(HasTitle == 1, "True", "False")
  )
...
print(ggplot(title_summary, aes(x = Type, y = Proportion, fill = title_Status)) +
        ...
        labs(
          title = "Proporzione di HasTitle per categoria",
          x = "Tipo di URL",
          y = "Proporzione",
          fill = "Stato HasTitle"
        ) +
