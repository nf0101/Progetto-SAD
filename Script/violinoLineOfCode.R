...
# Creare un grafico a violino per la distribuzione di 'TLDLength' con mediana e quartili
print(ggplot(combined_dataset_clean, aes(x = factor(label, labels = c("Phishing", "Legittimi")), 
                             y = TLDLength, 
  ...
  scale_y_continuous(
    ...
    breaks = c(0, 10, 100, 1000, 10000, 100000, 500000),       # Specifica i punti della scala
    labels = c("0", "10", "100", "1000", "10k", "100k", "500k")         # Etichette desiderate
  ) +
  # Aggiungi mediana
  ...
  labs(
    title = "Distribuzione di TLDLength per Tipo di URL",
    x = "Tipo di URL",
    y = "TLDLength",
    fill = "Tipo di URL"
  ) +
