...
for (col in columns_to_plot) {
  ...
  p <- ggplot(combined_dataset, aes(x = factor(label, labels = c("Phishing", "Legittimi")), 
   ...
   # scale_y_continuous(
    ...
    # Aggiungi cerchietti rossi per valori > 10
    geom_jitter(
      data = combined_dataset[combined_dataset[[col]] >= 1, ],  # Subset dei dati
      aes(x = "Phishing", y = .data[[col]]),  # Mappatura corretta delle estetiche
      color = "red", fill = NA, shape = 21, size = 2, width = 0.2, alpha=0.4 # Cerchietti rossi vuoti
    ) + # Trasparenza dei punti
