...
tld_summary <- combined_dataset %>%
  group_by(TLD, label) %>%
  summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = label, values_from = count, values_fill = list(count = 0)) %>%
  rename(Legittimo = `1`, Phishing = `0`)

# Ordina i TLD per il numero di siti di phishing e seleziona i primi 50
top_50_tlds <- tld_summary %>%
  arrange(desc(Phishing)) %>%
  head(50)

# Trasforma i dati in formato lungo per poter fare il grafico a barre affiancate
tlds_long <- top_50_tlds %>%
  pivot_longer(cols = c("Phishing", "Legittimo"), names_to = "TipoSito", values_to = "Conteggio")

# Crea il grafico a barre affiancate per i primi 50 TLD
bar_plot <- ggplot(tlds_long, aes(x = reorder(TLD, -Conteggio), y = Conteggio, fill = TipoSito)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  labs(title = "Primi 50 TLD con siti di Phishing e Legittimi",
       x = "TLD",
       y = "Conteggio") +
